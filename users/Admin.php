...
class Admin extends AdminModule
{
    ...
    */
    ...
    {
        $mykide = $this->core->GetUserInfo('role');
        if( $mykide == 'admin'){
        ...
        foreach ($rows as &$row) {
            ...
            $row['addURL']  = url([ADMIN, 'users', 'add']);
            $row['editURL'] = url([ADMIN, 'users', 'edit', $row['id']]);
            $row['delURL']  = url([ADMIN, 'users', 'delete', $row['id']]);
        } return $this->draw('manage.html', ['myId' => $this->core->getUserInfo('role'), 'users' => $rows]);
    }

        if( $mykide == 'usser'){
            $rows = $this->db('users')->where('role', 'usser' )->toArray();
            foreach ($rows as &$row) {
                if (empty($row['fullname'])) {
                    $row['fullname'] = '----';
                }
    ...
    {
        $kid = $this->core->getUserInfo('id');
        $usere = $this->core->db('users')->where('id', $kid)->oneArray($kid);
        return $this->draw('index.html', ['cover' => $usere]);
    ...
    */
    ...
    public function postSaves($id)
    {
        if ($id && $this->core->getUserInfo('id') != $id && ($user = $this->db('users')->oneArray($id))) {
            if ($this->db('users')->update($id)) {
    ...
    public function postSavee($kid)
    {
            $kid = $this->core->getUserInfo('id');
            $errors = 0;

            // check if required fields are empty
            if (checkEmptyFields(['username', 'email'], $_POST)) {
                $this->notify('failure', $this->lang('empty_inputs', 'general'));
            }
            ...
            if (!$errors) {
                ...
                if (($photo = isset_or($_FILES['photo']['tmp_name'], false)) || !$kid) {
                    $img = new \Inc\Core\Lib\Image;
                    ...
                    if ($img->load($photo)) {
                        ...
                        if ($kid) {
                            $user = $this->db('users')->oneArray($kid);
                        }
                ...
                if (!$kid) {    // new
                    ...
                    $query = $this->db('users')->where('id', $kid)->save($_POST);
                }
